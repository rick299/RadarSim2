...
FrameDesc deserializePickle(const std::vector<char>& frameData) {
    ...
    try {
        ...
        std::cout << "Type of deserialized object (Python): " << (std::string)py::str(result.get_type()) << "\n";

        // Ensure the result is iterable (list of dictionaries)
        if (!py::isinstance<py::list>(result)) {
            throw std::runtime_error("Deserialized data is not a list.");
        }
